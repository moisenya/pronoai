...
function extractMarketOdds(competition, side) {
  ...
  const prioritized =
    oddsList.find((entry) => /moneyline|victoire/i.test(entry?.details ?? "")) ??
    oddsList[0];
  ...
  const provider =
    prioritized?.provider?.name ??
    prioritized?.provider?.id ??
    prioritized?.provider?.displayName ??
    null;

  const teamOdds = side === "home" ? prioritized?.homeTeamOdds : prioritized?.awayTeamOdds;
  ...
  return {
    ...
    provider,
    detail: prioritized?.details ?? null,
  };
}
...
function buildDynamicPick(event, sport, market) {
  ...
  const marketSnapshot = extractMarketOdds(competition, side);
  const marketProbability =
    marketSnapshot && marketSnapshot.decimalOdds > 1
      ? Math.round((1 / marketSnapshot.decimalOdds) * 100)
      : null;
  ...
  if (marketSnapshot) {
    analysisParts.push(
      `Le marché${marketSnapshot.provider ? ` ${marketSnapshot.provider}` : ""} cote ${pickName} à ${marketSnapshot.decimalOdds.toFixed(
        2
      )}${marketProbability ? ` (~${marketProbability}% implicite)` : ""}${
        marketSnapshot.detail ? ` (${marketSnapshot.detail})` : ""
      }.`
    );
  }
  if (pickRecord) {
  ...
  return {
    ...
    confidence,
    odds: modelOdds,
    marketOdds: marketSnapshot?.decimalOdds ?? null,
    marketProvider: marketSnapshot?.provider ?? null,
    marketDetail: marketSnapshot?.detail ?? null,
